...
from utils import to_gpu, get_mask_from_lengths, to_gpu_graph
...
class Tacotron2(nn.Module):
    def __init__(self, hparams):
        ...
        self.embedding = nn.Embedding(
            hparams.n_character_symbols, hparams.character_embedding_dim)
        std = sqrt(2.0 / (hparams.n_character_symbols + hparams.character_embedding_dim))
...
class Semantic_Tacotron2(nn.Module):
    def __init__(self, hparams):
        ...
        self.bert_dim = hparams.bert_dim
        self.bert = BertModel.from_pretrained('./bert/bert-base-uncased')

        self.deprels_num = hparams.deprels_num
        if hparams.dep_graph_type.startswith('bi'):
            self.gnn_uni = GGCN(self.bert_dim, self.bert_dim, self.deprels_num)
            self.gnn_rev = GGCN(self.bert_dim, self.bert_dim, self.deprels_num)
        else:
            self.gnn = GGCN(self.bert_dim, self.bert_dim, self.deprels_num)

        self.tacotron2 = Tacotron2(hparams)

    def parse_batch(self, batch):

        input_lengths, inputs_padded, words_padded, mapping_padded, g_batch, mel_padded, gate_padded, output_lengths = batch
        ...
        return (
            (input_lengths, inputs_padded, words_padded, mapping_padded, g_batch, mel_padded, max_len, output_lengths),
            (mel_padded, gate_padded))
    ...
    def forward(self, inputs):
        ...
        if hparams.dep_graph_type.startswith('bi'):
            g_output = self.gnn_uni(g_batch, g_nfeat, g_etype) + self.gnn_rev(dgl.reverse(g_batch), g_nfeat, g_etype)
        else:
            g_output = self.gnn(g_batch, g_nfeat, g_etype)

        bert_inputs_batch = to_gpu(torch.FloatTensor(batch_size, max_len, self.bert_dim)).float()

        for i in range(batch_size):
            for j in range(max_len):
                bert_inputs_batch[i][j] = g_output[mapping_padded[i][j]]

        tacotron2_inputs = (inputs_padded, input_lengths, mel_padded, max_len, output_lengths)
    ...
    def inference(self, inputs):
        ...
        bert_inputs_batch = to_gpu(torch.FloatTensor(batch_size, input_lengths[0], self.bert_dim)).float()
        for i in range(batch_size):
            for j in range(input_lengths[0]):
                bert_inputs_batch[i][j] = g_output[mapping_padded[i][j]]

        tacotron2_inputs = inputs_padded
        tacotron2_outputs = self.tacotron2.inference(tacotron2_inputs, bert_inputs_batch)
